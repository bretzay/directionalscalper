...
def create_dict(source: dict, prev_key, config = None) -> dict:
    destination = {
        key:check_value(key,value,config, prev_key) if key in IGNORE_CATEGORY
        else create_dict(value, key, config) if isinstance(value,dict)
        
        else [
            create_dict(item, key, config) if isinstance(item,dict)
            else None
            for item in value
        ]
        if isinstance(value,list)
        
        else check_value(key, value, config, prev_key) if config is not None
        else value
...
def check_value(i_key, i_value, config, prev_key) -> str | int | float | bool:
    stack = [config]
    while stack:
        d = stack.pop()
        if(prev_key):
            if prev_key not in d :
                continue
            if i_key in d[prev_key]:
                return d[prev_key][i_key]
        if i_key in d:
            return d[i_key]
...
def main():
    ...
    with open(config_path) as config_f, open(EXAMPLE_FILE) as example_f, open(config_path+".new", "w") as new:
        ...
        new_dict: dict = {}
        new_dict = create_dict(example, None, config)
        json.dump(new_dict, new, indent = 4)
    print("Your file has been successfuly updated.")
    print("""
If the file already existed, look for the new filename.json.new. It's up to you to check if everything seems ok and to rename it to filename.json to replace the previous file.
